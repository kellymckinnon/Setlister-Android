...
/**
 * {@link IdlingResource} which monitors a {@link View} for a given visibility state. The resource
 * is considered idle when the View has the desired state.
 *
...
public class ViewVisibilityIdlingResource implements IdlingResource {
  ...
  /**
   * Hold weak reference to the View, so we don't leak memory even if the resource isn't
   * unregistered.
   */
   ...
   */
  public ViewVisibilityIdlingResource(
      @NonNull Activity activity, @IdRes int viewId, int visibility) {
  ...
  public ViewVisibilityIdlingResource(@NonNull View view, int visibility) {
    ...
    mName =
        "View Visibility for view " + view.getId() + "(@" + System.identityHashCode(mView) + ")";
  }
